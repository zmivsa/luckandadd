...
// 非凌晨0点-0点56分时候运行的账号
$.luckDrawRunNum = process.env.LUCK_DRAW_RUN_NUM ? process.env.LUCK_DRAW_RUN_NUM : 5;
// 凌晨0点-0点56分时候运行的账号
$.luckDrawRunNum2 = process.env.LUCK_DRAW_RUN_NUM2 ? process.env.LUCK_DRAW_RUN_NUM2 : 4;
...
$.addressArray = [
    "湖北省,荆门市,掇刀区,团林铺镇长城汽车荆门分公司一号门,13089650058,300191,420804, 赵瑞"
]
...
!(async () => {
    ...
    if ($.skipCk != '') {
        for (let skipCk of $.skipCk.split("&")) {
            for (let cookie of cookiesArr) {
                if (cookie.indexOf(skipCk) != -1) {
                    removeItem(cookiesArr, cookie)
                    console.log(`\n✅ 跳过指定cookie【${skipCk}】成功`)
                }
            }
        }
    }
    ...
    for (let i = 0; i < $.runNum; i++) {
        ...
        console.log(`\n⌛️ 等待3秒`)
        waitTime = parseInt(Math.random() * 2000 + 3000, 10)
        await $.wait(waitTime)
...
async function jdmodule() {
    ...
    await takePostRequest("shopInfo")
       if ($.luckDrawOpenCard == 'true') {
          console.log(`💳 已设置开卡为true，去入会`)
          await opencard()
          console.log(`================`)
        } else {
                    ...
                    if (res.result && res.result === true) {
                       ...
                       if ($.index == 1 &&  $.prizeMsg == '') {
                            ...
                            for (let prizeInfo of $.content) {
                                if (prizeInfo.name.indexOf("谢谢") != -1 || prizeInfo.name.indexOf("再来") != -1 ) {
                                    continue;
                                } else {
            ...
            case 'start':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        ...
                        console.log(`${res.errorMessage}` || `空气 💭`)
                   $.message += `${res.errorMessage}` || `空气`+` ` 
                        }
...
function getshopactivityId() {
  return new Promise(async resolve => {
    let body = `{"venderId":"${$.venderId}","channel":406,"payUpShop":true}`
    //let h5st = 'undefined'
    //try {
